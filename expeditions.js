(function () {
  ...
  // Общая утилита: создание иконки с fallback
  function createShipIcon(src, alt, size = 20) {
  ...
  // Кешированные ссылки на DOM-элементы
  const els = {};
  ...
  function compute() {
    ...
    const hyperTechEl = el('tech_hyper-level');
    const percentResEl = el('percent-resources');
    const percentShipsEl = el('percent-ships');
    const classBonusCollectorEl = el('class-bonus-collector');
    const classBonusDiscovererEl = el('class-bonus-discoverer');
    const darkMatterBonusEl = el('dark-matter-discovery-bonus');
    const resourceBoosterEl = el('resource-discovery-booster');
    ...
    try { options.prm.fleet = JSON.stringify(createFleetJSON()); } catch (e) { /* ignore */ }
    ...
    // Сброс индикаторов "может быть найден"
    for (let d = 0; d < shipProperties.length; d++) {
      const canEl = el(`can${shipProperties[d][0]}`);
      if (canEl) {
    ...
    for (let d = 0; d < shipProperties.length; d++) {
      const count = parseInput(el(`num${shipProperties[d][0]}`));
      if (d > 2 && count > 0) {
        foundShips = true;
        for (let j = 3; j <= d; j++) {
          const canEl = el(`can${shipProperties[j][0]}`);
          if (canEl) {
        ...
        if (d < shipProperties.length - 1) {
          const nextEl = el(`can${shipProperties[d + 1][0]}`);
          if (nextEl) {
    ...
    for (let d = 3; d < shipProperties.length; d++) {
      const canEl = el(`can${shipProperties[d][0]}`);
      const findEl = el(`find${shipProperties[d][0]}`);
      if (canEl && findEl) {
  ...
  function initBonusesPanel() {
    ...
    shipsData.forEach((ship, i) => {
      ...
      const img = createShipIcon(`images/ships/${shipImageMap[shipKey]}`, getShipName(shipKey), 20);
  ...
  function initExpeditionsTable() {
    ...
    if (tbody.children.length === 0) {
      ...
      shipsData.forEach((ship, i) => {
        ...
        const shipKey = ship[0];
        const img = createShipIcon(`images/ships/${shipImageMap[shipKey]}`, getShipName(shipKey), 28);
        nameCell.appendChild(img);
        nameCell.appendChild(document.createTextNode(getShipName(shipKey)));
