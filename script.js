(function () {
    ...
    function invalidateDomCache(ids) {
        ids.forEach(id => { delete DOM_CACHE[id]; });
    }
    ...
    function geomSum(base, factor, from, to) {
        return cached(`geom_${base.m}_${base.c}_${base.d}_${factor}_${from}_${to}`, () => {
            ...
            return { m, c, d, points: Math.round((m + c + d) / 1000), levels: count };
        });
    }
    ...
    function recalcAllResearch() {
        ...
        tbody.querySelectorAll('tr').forEach(tr => {
            ...
            if (to - from > CONFIG.MAX_LEVEL_SPAN) to = from + CONFIG.MAX_LEVEL_SPAN;
            const sum = geomSum(data.base, data.factor, from, to);
            updateResourceCell(refs.m, sum.m, 'val-metal');
            updateResourceCell(refs.c, sum.c, 'val-crystal');
            updateResourceCell(refs.d, sum.d, 'val-deut');
            refs.p.textContent = formatNumberWithDots(sum.points);
            sm += sum.m; sc += sum.c; sd += sum.d; sp += sum.points; totalLevels += sum.levels;
        });
        ...
        const dict = getDict();
        $('tmTotal').textContent = `${dict.totalTMLabel || 'Итого:'} ${formatNumberWithDots(totalTM)}`;
        updateBoxesNeeded();
        updateSumAllTabsRows();
    }
    ...
    function renderTable() {
        ...
        invalidateDomCache(['sumMetalF', 'sumCrystalF', 'sumDeutF', 'sumTotalMetalF']);
        cachedAggrRows = null;
    ...
    function computeFleet() {
        ...
        const totalMetal = Math.round(convertToMetal(fm, fc, fd));
    ...
    function attachInputsHandlers() {
        ...
        const bindSubtabs = (selector, dataKey, panels, onSwitch, storageKey) => {
            document.querySelectorAll(selector).forEach(btn => {
                btn.addEventListener('click', ev => {
                    ev.stopPropagation();
                    const tab = btn.dataset[dataKey];
                    document.querySelectorAll(selector).forEach(b => b.classList.remove('active'));
                    btn.classList.add('active');
                    Object.entries(panels).forEach(([id, key]) => $(id)?.classList.toggle('active', tab === key));
                    onSwitch(tab);
        ...
        bindSubtabs('.building-subtab-btn', 'buildingTab',
            { planetBuildingsContent: 'planet', moonBuildingsContent: 'moon' },
            tab => { (tab === 'moon' ? recalcAllMoonBuildings : recalcAllBuildings)(); updateBoxesNeeded(); },
            'og_calc_active_building_tab');

        bindSubtabs('.lf-subtab-btn', 'subtab',
            { 'lf-buildings': 'lf-buildings', 'lf-research': 'lf-research' },
            tab => (tab === 'lf-research' ? recalcAllLfResearch : recalcAllLfBuildings)(),
            'og_calc_active_lf_subtab_v1');
        ...
        if (langToggle && langMenu) {
            ...
            Object.keys(LANGUAGES).forEach(code => {
                ...
                option.addEventListener('click', e => {
                    ...
                    e.preventDefault();
                    applyLang(code);
                    setTimeout(() => { langMenu.style.display = 'none'; }, 50);
                    setLangLabel(code);
                });
            });
    ...
    function persistLfInputs() {
        safeSet(`${KEYS.LF_INPUTS_BUILD}_${currentLifeformRace}`, JSON.stringify(collectRowData('#tbodyLfBuildings tr')));
        safeSet(`${KEYS.LF_INPUTS_RESEARCH}_${currentLifeformRace}`, JSON.stringify(collectRowData('#tbodyLfResearch tr')));
    }
    ...
    function saveAllInputsBeforeSwitch() {
        saveInputRowsFromSelector('#tbodyBuildings tr', KEYS.INPUTS_BUILD);
        saveInputRowsFromSelector('#tbodyResearch tr', KEYS.INPUTS_RESEARCH);
        saveInputRowsFromSelector('#tbodyMoonBuildings tr', KEYS.INPUTS_MOON_BUILD);
        saveInputRowsFromSelector('#tbodyLfBuildings tr', `${KEYS.LF_INPUTS_BUILD}_${currentLifeformRace}`);
        saveInputRowsFromSelector('#tbodyLfResearch tr', `${KEYS.LF_INPUTS_RESEARCH}_${currentLifeformRace}`);
        saveShipQuantities();
    ...
    function restoreAllInputsAfterSwitch() {
        restoreInputRowsFromSelector('#tbodyBuildings tr', KEYS.INPUTS_BUILD);
        restoreInputRowsFromSelector('#tbodyResearch tr', KEYS.INPUTS_RESEARCH);
        restoreInputRowsFromSelector('#tbodyMoonBuildings tr', KEYS.INPUTS_MOON_BUILD);
        restoreInputRowsFromSelector('#tbodyLfBuildings tr', `${KEYS.LF_INPUTS_BUILD}_${currentLifeformRace}`);
        restoreInputRowsFromSelector('#tbodyLfResearch tr', `${KEYS.LF_INPUTS_RESEARCH}_${currentLifeformRace}`);
    ...
    function applyLang(lang, skipRebuild = false) {
        ...
        if (activeTab === 'lifeforms') {
            ...
            $('lf-buildings')?.classList.toggle('active', activeLfSubtab === 'lf-buildings');
            $('lf-research')?.classList.toggle('active', activeLfSubtab === 'lf-research');
        }
    ...
    function setActiveTab(tab) {
        ...
        if (tab === 'fleet') { renderTable(); computeFleet(); }
        else if (tab === 'buildings') recalcAllBuildings();
            ...
            $('lf-buildings')?.classList.toggle('active', activeSub === 'lf-buildings');
            $('lf-research')?.classList.toggle('active', activeSub === 'lf-research');
            buildRowsLfBuildings();
            buildRowsLfResearch();
            restoreAllInputsAfterSwitch();
            attachLiveThousandsFormatting('#tbodyLfBuildings input, #tbodyLfResearch input');
    ...
    function switchView(newView) {
        ...
        if (newView === 'expeditions') window.panZoomExpeditions?.applyTransform();
        else window.panZoomMain?.applyTransform();
        safeSet('og_calc_active_view', newView);
        updateBackgroundVideo(newView);
    }

    function getSumAllTabsMetalValue() {
        let total = TOTALS.buildings.total + TOTALS.moonBuildings.total + TOTALS.research.total + TOTALS.fleet.total;
    ...
    function getAggrRows() {
        if (!cachedAggrRows) {
            cachedAggrRows = {
                ...
                regularRows: Array.from(document.querySelectorAll('.regular-total-row')),
    ...
    function updateBoxesCostTL(targetMetal = null) {
        ...
        if (currencyEl) currencyEl.textContent = formatNumberWithDots(Math.round(amountInBYN * targetRate));
    ...
    function updateLfBonusesVisibility(race) {
        ...
        invalidateDomCache(BONUS_INPUT_IDS);
        const fields = {
    ...
    function initPanZoom() {
        if (!panZoomMain) panZoomMain = new PanZoomController('tableWrapper', 'dragHandle', KEYS.TRANSFORM);
        if (!panZoomExpeditions) panZoomExpeditions = new PanZoomController('expeditionsWrapper', 'dragHandleExpeditions', 'og_calc_expeditions_transform');
    ...
    function initZoomControls() {
        const getActiveController = () => {
            const expWrapper = $('expeditionsWrapper');
            return expWrapper && expWrapper.style.display !== 'none' ? window.panZoomExpeditions : window.panZoomMain;
        };
        ...
        $('globalZoomReset')?.addEventListener('click', () => {
            ...
            if (isExp) {
                ...
                const uSpeed = $('universe-speed');
                const rBooster = $('resource-discovery-booster');
                const hTop = $('highTop');
                if (pClass) pClass.selectedIndex = 0;
                if (uSpeed) uSpeed.selectedIndex = 0;
                if (rBooster) rBooster.selectedIndex = 0;
                if (hTop) hTop.selectedIndex = 0;
    ...
    function fullResetToZero() {
        ...
        RACES.forEach(race => {
            ...
            saveInputRowsFromSelector('#tbodyLfBuildings tr', `${KEYS.LF_INPUTS_BUILD}_${race}`);
            saveInputRowsFromSelector('#tbodyLfResearch tr', `${KEYS.LF_INPUTS_RESEARCH}_${race}`);
            recalcAllLfBuildings();
            recalcAllLfResearch();
        });
    ...
    function initApp() {
        ...
        $('lf-buildings')?.classList.toggle('active', savedLfSubtab === 'lf-buildings');
        $('lf-research')?.classList.toggle('active', savedLfSubtab === 'lf-research');
