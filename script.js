...
// ✅ ИСПРАВЛЕНО: Хранилище кристалла (индекс 9) - base: { m: 1000, c: 500 } вместо { m: 2000, c: 1000 }
const BUILDINGS_DATA = [
    { base: { m: 60, c: 15, d: 0 }, factor: 1.5 },      // 0: Рудник металла
    { base: { m: 48, c: 24, d: 0 }, factor: 1.6 },      // 1: Рудник кристалла
    { base: { m: 225, c: 75, d: 0 }, factor: 1.5 },     // 2: Синтезатор дейтерия
    { base: { m: 75, c: 30, d: 0 }, factor: 1.5 },      // 3: Солнечная электростанция
    { base: { m: 900, c: 360, d: 180 }, factor: 1.8 },  // 4: Термоядерная электростанция
    { base: { m: 400, c: 120, d: 200 }, factor: 2.0 },  // 5: Фабрика роботов
    { base: { m: 1000000, c: 500000, d: 100000 }, factor: 2.0 }, // 6: Фабрика нанитов
    { base: { m: 400, c: 200, d: 100 }, factor: 2.0 },  // 7: Верфь
    { base: { m: 1000, c: 0, d: 0 }, factor: 2.0 },     // 8: Хранилище металла
    { base: { m: 1000, c: 500, d: 0 }, factor: 2.0 },   // 9: Хранилище кристалла ✅ ИСПРАВЛЕНО
    { base: { m: 1000, c: 1000, d: 0 }, factor: 2.0 },  // 10: Хранилище дейтерия
    { base: { m: 200, c: 400, d: 200 }, factor: 2.0 },  // 11: Исследовательская лаборатория
    { base: { m: 0, c: 50000, d: 100000 }, factor: 2.0 }, // 12: Терраформер
    { base: { m: 20000, c: 40000, d: 0 }, factor: 2.0 }, // 13: Склад альянса
    { base: { m: 200, c: 0, d: 50 }, factor: 5.0 },     // 14: Космический док
    { base: { m: 20000, c: 20000, d: 1000 }, factor: 2.0 } // 15: Ракетная шахта
];
...
function getCachedCalculation(key, fn) {
...
function sanitizeInput(input) {
    if (!input) return '';
    return String(input).replace(/[^0-9]/g, '').slice(0, 15);
}
...
function updateLfBonusesVisibility(race) {
    ...
    fieldsToCreate.forEach(field => {
        ...
        if (existingValue && existingValue !== '') {
            input.value = sanitizeInput(existingValue);
        } else {
...
function computeFleet() {
    try {
        ...
        const planetM = parseNumberInput(sanitizeInput(document.getElementById('planetMetal')?.value));
        const planetC = parseNumberInput(sanitizeInput(document.getElementById('planetCrystal')?.value));
        const planetD = parseNumberInput(sanitizeInput(document.getElementById('planetDeut')?.value));
        ...
        const boxesCount = parseNumberInput(sanitizeInput(document.getElementById('boxesCount')?.value));
        ...
        safeLocalStorageSet(KEYS.BOXES, JSON.stringify(boxesData));
        updateBoxesNeeded();
...
function buildRowsBuildings() {
    ...
    names.forEach((name, i) => {
        ...
        planetInput.dataset.type = 'planets';
        planetInput.dataset.index = i;
        planetInput.inputmode = 'numeric';
        planetInput.value = '1';
        tdPlanets.appendChild(planetImg);
        tdPlanets.appendChild(planetInput);
        const tdM = document.createElement('td');
        tdM.className = 'm';
...
function attachInputsHandlers() {
    ...
    if (tbodyB) {
        tbodyB.addEventListener('input', e => {
            ...
            try {
                ...
                safeLocalStorageSet(KEYS.INPUTS_BUILD, JSON.stringify(rows));
            } catch (e) { console.warn('Save inputs build:', e); }
        });
    ...
    if (tbodyR) {
        tbodyR.addEventListener('input', e => {
            ...
            try {
                ...
                safeLocalStorageSet(KEYS.INPUTS_RESEARCH, JSON.stringify(rows));
            } catch (e) { console.warn('Save inputs research:', e); }
        });
    ...
    if (tmEl) {
        ...
        tmEl.addEventListener('change', () => {
            safeLocalStorageSet(KEYS.TM, tmEl.value);
        });
    }
...
function persistLfInputs() {
    try {
        ...
        buildRows.forEach((tr, i) => {
            const from = parseNumberInput(sanitizeInput(tr.querySelector('input[data-type="from"]')?.value));
            const to = parseNumberInput(sanitizeInput(tr.querySelector('input[data-type="to"]')?.value));
            const planets = parseNumberInput(sanitizeInput(tr.querySelector('input[data-type="planets"]')?.value)) || 1;
            b[i] = { from, to, planets };
        });
        ...
        safeLocalStorageSet(KEYS.LF_INPUTS_BUILD, JSON.stringify(b));
        safeLocalStorageSet(KEYS.LF_INPUTS_RESEARCH, JSON.stringify(r));
...
function applyLang(lang) {
    ...
    navBtns.forEach(btn => {
        const key = btn.dataset.view === 'costs' ? 'tabCosts' : 'tabExpeditions';
...
function saveAllInputsBeforeSwitch() {
    try {
        ...
        saveShipQuantities();
        const boxes = {
            boxesCount: parseNumberInput(sanitizeInput(document.getElementById('boxesCount')?.value)),
            boxValue: parseNumberInput(sanitizeInput(document.getElementById('boxValue')?.value)),
            planetMetal: parseNumberInput(sanitizeInput(document.getElementById('planetMetal')?.value)),
            planetCrystal: parseNumberInput(sanitizeInput(document.getElementById('planetCrystal')?.value)),
            planetDeut: parseNumberInput(sanitizeInput(document.getElementById('planetDeut')?.value))
        };
...
function restoreFromStorage() {
    try {
        ...
        const lfInputsBuild = JSON.parse(safeLocalStorageGet(KEYS.LF_INPUTS_BUILD, 'null'));
        if (lfInputsBuild) {
        ...
        const lfInputsResearch = JSON.parse(safeLocalStorageGet(KEYS.LF_INPUTS_RESEARCH, 'null'));
        if (lfInputsResearch) {
        ...
        const trf = JSON.parse(safeLocalStorageGet(KEYS.TRANSFORM, 'null'));
        if (trf) {
...
function centerWrapper() {
    ...
    safeLocalStorageSet(KEYS.TRANSFORM, JSON.stringify({
        scale: window.scale,
        posX: window.posX,
        posY: window.posY
    }));
...
function centerExpeditionsWrapper() {
    const wrapperEl = document.getElementById('expeditionsWrapper');
    if (!wrapperEl) return;
    window.scale = 1;
    wrapperEl.style.transform = 'none';
    const rect = wrapperEl.getBoundingClientRect();
    const viewportWidth = window.innerWidth;
    const posX = Math.round((viewportWidth - rect.width) / 2);
    const posY = Math.round(50);
    wrapperEl.style.transform = `translate(${posX}px, ${posY}px) scale(${window.scale})`;
    wrapperEl.style.willChange = 'transform';
    safeLocalStorageSet('og_calc_expeditions_transform', JSON.stringify({
        scale: window.scale,
        posX: posX,
        posY: posY
    }));
...
function fullResetToZero() {
    try {
        const currentTransform = safeLocalStorageGet(KEYS.TRANSFORM, null);
        const currentExpeditionsTransform = safeLocalStorageGet('og_calc_expeditions_transform', null);
        ...
        document.getElementById('boxesCostTL').innerHTML = 'TRY: —';
        document.getElementById('leftoverTmValue').textContent = '—';
        if (currentTransform) {
            safeLocalStorageSet(KEYS.TRANSFORM, currentTransform);
        }
        if (currentExpeditionsTransform) {
            safeLocalStorageSet('og_calc_expeditions_transform', currentExpeditionsTransform);
        }
...
function updateBoxesCostTL(targetMetal = null) {
    try {
        ...
        const bynValue = formatNumberWithDots(Math.round(totalTRY / 15.13));
...
function initDraggableWrapper(wrapperId, handleId, storageKey) {
    ...
    handle.addEventListener('pointerdown', (ev) => {
        ...
        if (transform && transform !== 'none') {
            ...
            startPosX = matrix.e; startPosY = matrix.f;
        } else {
            startPosX = parseFloat(style.left) || 0;
            startPosY = parseFloat(style.top) || 0;
        }
    ...
    document.addEventListener('pointermove', (ev) => {
        ...
        wrapper.style.transform = `translate(${Math.round(startPosX + dx)}px, ${Math.round(startPosY + dy)}px) scale(${window.scale || 1})`;
    ...
    const stopDrag = (ev) => {
        ...
        safeLocalStorageSet(storageKey, JSON.stringify({ left: posX, top: posY }));
        positionTabs();
    };
    document.addEventListener('pointerup', stopDrag);
    document.addEventListener('pointercancel', stopDrag);
    try {
        const pos = JSON.parse(safeLocalStorageGet(storageKey, '{}'));
        if (pos.left != null && pos.top != null) {
            wrapper.style.transform = `translate(${Math.round(pos.left)}px, ${Math.round(pos.top)}px) scale(${window.scale || 1})`;
            wrapper.style.position = 'absolute';
            ['left','top','right','bottom'].forEach(p => wrapper.style[p] = 'auto');
        }
    } catch (e) { console.warn('Load drag pos:', e); }
}

function initZoomControls() {
    const zoomIn = document.getElementById('globalZoomIn');
    const zoomOut = document.getElementById('globalZoomOut');
    const zoomReset = document.getElementById('globalZoomReset');
    const getActiveWrapper = () => {
        const isExpeditions = document.getElementById('expeditionsWrapper')?.style.display !== 'none';
        return {
            element: isExpeditions ? document.getElementById('expeditionsWrapper') : document.getElementById('tableWrapper'),
            storageKey: isExpeditions ? 'og_calc_expeditions_transform' : 'og_calc_transform_v2'
        };
    };
    const loadTransform = (wrapperInfo) => {
        try {
            const saved = safeLocalStorageGet(wrapperInfo.storageKey, null);
            if (saved) {
            ...
            console.error('Error loading transform:', e);
        }
        return { scale: 1, posX: 0, posY: 0 };
    };
    const saveTransform = (wrapperInfo, transform) => {
        safeLocalStorageSet(wrapperInfo.storageKey, JSON.stringify(transform));
    };
    const applyTransform = (wrapper, transform) => {
        wrapper.style.transform = `translate(${Math.round(transform.posX)}px, ${Math.round(transform.posY)}px) scale(${transform.scale})`;
        wrapper.style.willChange = 'transform';
    };
    zoomIn?.addEventListener('click', () => {
        const wrapperInfo = getActiveWrapper();
        const wrapper = wrapperInfo.element;
        if (!wrapper) return;
        let transform = loadTransform(wrapperInfo);
        const newScale = Math.min(3.5, transform.scale * 1.1);
        transform.scale = newScale;
        applyTransform(wrapper, transform);
        saveTransform(wrapperInfo, transform);
    });
    zoomOut?.addEventListener('click', () => {
        ...
        if (!wrapper) return;
        let transform = loadTransform(wrapperInfo);
        const newScale = Math.max(0.3, transform.scale / 1.1);
        transform.scale = newScale;
        applyTransform(wrapper, transform);
        saveTransform(wrapperInfo, transform);
    });
    zoomReset?.addEventListener('click', () => {
        ...
        if (isExpeditions) {
            ...
            if (classEl) classEl.value = '0';
            const speedEl = document.getElementById('universe-speed');
            if (speedEl) speedEl.value = '1';
            const boosterEl = document.getElementById('resource-discovery-booster');
            if (boosterEl) boosterEl.value = '0';
            const highTopEl = document.getElementById('highTop');
            if (highTopEl) highTopEl.value = '5000000';
            document.querySelectorAll('#lf-ships-bonuses .lf-bonus-input').forEach(inp => {
                inp.value = '0';
            });
...
function initApp() {
    try {
        ...
        ['planetMetal', 'planetCrystal', 'planetDeut'].forEach(id => {
            ...
            if (el) {
                const handler = () => {
                    ...
                    try {
                        const boxes = JSON.parse(safeLocalStorageGet(KEYS.BOXES, '{}'));
                        boxes[id] = parseNumberInput(sanitizeInput(el.value));
                        safeLocalStorageSet(KEYS.BOXES, JSON.stringify(boxes));
                    } catch (e) { console.warn('Save planet resources:', e); }
                };
                el.addEventListener('input', handler);
                el.addEventListener('change', handler);
                el.addEventListener('blur', handler);
        ...
        attachInputsHandlers();
        restoreFromStorage();
        initDraggableWrapper('tableWrapper', 'dragHandle', KEYS.TRANSFORM);
        initDraggableWrapper('expeditionsWrapper', 'dragHandleExpeditions', 'og_calc_expeditions_pos');
        initZoomControls();
        ...
        document.getElementById('tableWrapper').style.display = savedView === 'costs' ? 'block' : 'none';
        document.getElementById('expeditionsWrapper').style.display = savedView === 'expeditions' ? 'block' : 'none';
        document.querySelectorAll('.nav-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                document.querySelectorAll('.nav-btn').forEach(b => b.classList.remove('active'));
                btn.classList.add('active');
                const view = btn.dataset.view;
                document.getElementById('tableWrapper').style.display = view === 'costs' ? 'block' : 'none';
                document.getElementById('expeditionsWrapper').style.display = view === 'expeditions' ? 'block' : 'none';
                safeLocalStorageSet('og_calc_active_view', view);
            });
        });
        const savedViewOnInit = safeLocalStorageGet('og_calc_active_view', 'costs');
        if (savedViewOnInit === 'expeditions') {
            setTimeout(() => {
                if (typeof window.initExpeditionUI === 'function') {
                    window.initExpeditionUI();
                } else {
                    document.dispatchEvent(new CustomEvent('initExpeditionOnLoad'));
                }
            }, 150);
        }
