...
const formatWithDotsRaw = (inputStr) => {
...
const createImageFallbackEl = (label) => {
    const span = document.createElement('span');
...
// ============================================
const geomSum = (base, factor, from, to) => {
...
// ============================================
// РАСЧЁТЫ ФОРМ ЖИЗНИ - ПОСТРОЙКИ
// ============================================
const calcBuildCostLF = (techID, techLevel, techData, costRdc) => {
...
const getBuildCostLF = (techID, techLevelFrom, techLevelTo, techData, ionTechLevel, rsrCostRdc, bldCostRdc = 0) => {
    let totalCost = [0, 0, 0];
...
const calcDeconstrCostLF = (techID, techLevel, techData, ionTechLevel) => {
    const cost = [0, 0, 0];
...
const getBuildEnergyCostLF = (techID, techLevel, techData, ionTechLevel, bldCostRdc = 0) => {
...
// ============================================
// ОБНОВЛЕНИЕ ВИДИМОСТИ БОНУСОВ
// ============================================
const updateLfBonusesVisibility = (race) => {
...
// ============================================
// РАСЧЁТЫ - ПОСТРОЙКИ ФЖ
// ============================================
const recalcAllLfBuildings = () => {
    ...
    tbody.querySelectorAll('tr').forEach(tr => {
        ...
        if (currentLifeformRace === 'rocktal') {
            ...
            const techIdMod = techId % 1000;
            if (techIdMod >= 1 && techIdMod <= 4 || techIdMod === 12 || techIdMod === 2001 || techIdMod === 2002) {
                bldCostRdc += 0.005 * mineralCenterLevel;
            }
        }
...
// ============================================
// РАСЧЁТЫ - ИССЛЕДОВАНИЯ ФЖ
// ============================================
const recalcAllLfResearch = () => {
...
// ============================================
// РАСЧЁТЫ - ОБЫЧНЫЕ ПОСТРОЙКИ
// ============================================
const recalcAllBuildings = () => {
...
// ============================================
// РАСЧЁТЫ - ОБЫЧНЫЕ ИССЛЕДОВАНИЯ
// ============================================
const recalcAllResearch = () => {
...
const computeFleet = () => {
    try {
        ...
        console.error('Error in computeFleet:', e);
    }
};

// ============================================
// ОТРИСОВКА ТАБЛИЦЫ КОРАБЛЕЙ
// ============================================
const renderTable = () => {
    try {
...
// ============================================
// ПОСТРОЕНИЕ СТРОК ТАБЛИЦ
// ============================================
const buildRowsBuildings = () => {
    const tbodyB = document.getElementById('tbodyBuildings');
    if (!tbodyB) return;
    tbodyB.innerHTML = '';
...
const attachLiveThousandsFormatting = (selector) => {
...
const attachInputsHandlers = () => {
    ...
    if (langToggle && langDropdownMenu) {
        ...
        availableLangs.forEach(langCode => {
            ...
            iconImg.src = LANGUAGE_CONFIG.ICONS[langCode] || 'images/languages/unknown.png';
            iconImg.alt = LANGUAGE_CONFIG.NAMES[langCode] || langCode.toUpperCase();
            ...
            textSpan.textContent = LANGUAGE_CONFIG.NAMES[langCode] || langCode.toUpperCase();
            ...
            option.addEventListener('click', (e) => {
                ...
                newIconImg.src = LANGUAGE_CONFIG.ICONS[langCode] || 'images/languages/unknown.png';
                newIconImg.alt = LANGUAGE_CONFIG.NAMES[langCode] || langCode.toUpperCase();
                ...
                newTextSpan.textContent = LANGUAGE_CONFIG.NAMES[langCode] || langCode.toUpperCase();
        ...
        if (currentLangEl) {
            currentLangEl.innerHTML = '';
            const iconImg = document.createElement('img');
            iconImg.src = LANGUAGE_CONFIG.ICONS[currentLang] || 'images/languages/unknown.png';
            iconImg.alt = LANGUAGE_CONFIG.NAMES[currentLang] || currentLang.toUpperCase();
            iconImg.className = 'lang-icon-img';
            iconImg.style.cssText = 'width:18px;height:18px;vertical-align:middle;margin-right:4px';
            
            const textSpan = document.createElement('span');
            textSpan.className = 'lang-text';
            textSpan.textContent = LANGUAGE_CONFIG.NAMES[currentLang] || currentLang.toUpperCase();
...
const attachBonusInputHandlers = () => {
    BONUS_INPUT_IDS.forEach(id => {
        const el = document.getElementById(id);
...
const saveAllInputsBeforeSwitch = () => {
    try {
...
const restoreAllInputsAfterSwitch = () => {
    try {
...
const getCurrentTotalMetalValue = () => {
    try {
...
const updateBoxesCostTL = (targetMetal = null) => {
    try {
        ...
        const bynValue = formatNumberWithDots(Math.round(totalTRY / 17.70));
...
// DRAG & DROP
// ============================================
const initDraggableWrapper = (wrapperId, handleId, storageKey) => {
...
const initZoomControls = () => {
    ...
    window.posX = window.posX || 0;
    window.posY = window.posY || 0;
    ...
    const getActiveWrapper = () => {
        ...
        return {
            ...
            storageKey: isExpeditions ? 'og_calc_expeditions_transform' : KEYS.TRANSFORM,
            isExpeditions: isExpeditions
        };
    };
    
    const applyTransform = (wrapper, scale, posX, posY) => {
        if (!wrapper) return;
        wrapper.style.transform = `translate(${Math.round(posX)}px, ${Math.round(posY)}px) scale(${scale})`;
        wrapper.style.transformOrigin = 'center center';
        wrapper.style.willChange = 'transform';
    };
    
    const saveTransform = (wrapperInfo, scale, posX, posY) => {
        try {
            localStorage.setItem(wrapperInfo.storageKey, JSON.stringify({ scale, posX, posY }));
        } catch (e) {
    ...
    const loadTransform = (wrapperInfo) => {
        try {
            const saved = localStorage.getItem(wrapperInfo.storageKey);
            if (saved) {
                const data = JSON.parse(saved);
                return { scale: data.scale || 1, posX: data.posX || 0, posY: data.posY || 0 };
            }
        } catch (e) {
    ...
    const centerWrapper = (wrapper) => {
        if (!wrapper) return { posX: 0, posY: 0 };
        const rect = wrapper.getBoundingClientRect();
        const viewportWidth = window.innerWidth;
        const viewportHeight = window.innerHeight;
        const wrapperWidth = rect.width * window.scale;
        const wrapperHeight = rect.height * window.scale;
        return {
            posX: Math.round((viewportWidth - wrapperWidth) / 2),
            posY: Math.round((viewportHeight - wrapperHeight) / 2)
        };
    };
    ...
    zoomIn?.addEventListener('click', () => {
        ...
        const current = loadTransform(wrapperInfo);
        const newScale = Math.min(3.5, parseFloat((current.scale * 1.1).toFixed(2)));
        const centerPos = centerWrapper(wrapper);
        
        window.scale = newScale;
        window.posX = centerPos.posX;
        window.posY = centerPos.posY;
        
        applyTransform(wrapper, newScale, centerPos.posX, centerPos.posY);
        saveTransform(wrapperInfo, newScale, centerPos.posX, centerPos.posY);
    });
    ...
    zoomOut?.addEventListener('click', () => {
        ...
        const current = loadTransform(wrapperInfo);
        const newScale = Math.max(0.3, parseFloat((current.scale / 1.1).toFixed(2)));
        const centerPos = centerWrapper(wrapper);
    ...
    zoomReset?.addEventListener('click', () => {
        ...
        if (wrapper) {
            ...
            window.posX = centerPos.posX;
            window.posY = centerPos.posY;
            applyTransform(wrapper, 1, centerPos.posX, centerPos.posY);
            saveTransform(wrapperInfo, 1, centerPos.posX, centerPos.posY);
        }
    ...
    setTimeout(() => {
        ...
        const wrapper = wrapperInfo.element;
        if (wrapper) {
            const saved = loadTransform(wrapperInfo);
            window.scale = saved.scale;
            window.posX = saved.posX;
            window.posY = saved.posY;
            
            if (saved.scale === 1 && saved.posX === 0 && saved.posY === 0) {
                const centerPos = centerWrapper(wrapper);
                window.posX = centerPos.posX;
                window.posY = centerPos.posY;
            }
            
            applyTransform(wrapper, window.scale, window.posX, window.posY);
        }
    }, 100);
};
...
const centerExpeditionsWrapper = () => {
    ...
    try {
        localStorage.setItem('og_calc_expeditions_transform', JSON.stringify({
            scale: window.scale,
            posX: posX,
            posY: posY
        }));
...
const resetAllData = () => {
    try {
        document.querySelectorAll('input[type="text"], input[type="number"], input[type="numeric"]').forEach(input => {
            input.value = '';
        });
        ...
        Object.values(KEYS).forEach(k => {
            if (k !== KEYS.LANG && k !== KEYS.LF_RACE && k !== KEYS.ACTIVE_TAB) {
        ...
        BONUS_INPUT_IDS.forEach(id => {
            const el = document.getElementById(id);
            if (el) el.value = '';
            try { localStorage.removeItem(`og_calc_${id}`); } catch (e) {}
        });
        
        isSumAllTabsMode = false;
        const checkbox = document.getElementById('sumAllTabsCheckbox');
        if (checkbox) checkbox.checked = false;
        try { localStorage.removeItem(KEYS.SUM_ALL_TABS); } catch (e) {}
        ...
        expeditionFields.forEach(id => {
            const el = document.getElementById(id);
            if (el) el.value = '0';
        });
        
        const expeditionSelects = ['player-class', 'universe-speed', 'resource-discovery-booster', 'highTop'];
        expeditionSelects.forEach(id => {
            const el = document.getElementById(id);
            if (el) {
                if (id === 'player-class') el.value = '0';
                else if (id === 'universe-speed') el.value = '1';
                else if (id === 'resource-discovery-booster') el.value = '0';
                else if (id === 'highTop') el.value = '5000000';
            }
        });
        ...
        console.error('Error in resetAllData:', e);
    }
};
...
const restoreFromStorage = () => {
    try {
        ...
        console.error('Error in restoreFromStorage:', e);
    }
};
    ...
    try {
        ...
        document.querySelectorAll('.nav-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                localStorage.setItem('og_calc_active_view', view);
            });
        });
        
        const savedViewOnInit = localStorage.getItem('og_calc_active_view') || 'costs';
