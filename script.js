(function() {
  ...
  const TECH_COSTS_LF = {
    ...
    1105: [25000, 20000, 10000, 0, 4500, 1.4, 1.4, 1.4, 0, 1.2], 1106: [35000, 25000, 15000, 0, 5000, 1.5, 1.5, 1.5, 0, 1.3],
  ...
  const KEYS = {
    LANG: 'og_calc_lang_v2', TRANSFORM: 'og_calc_transform_v2',
    INPUTS_BUILD: 'og_calc_inputs_build_v2', INPUTS_RESEARCH: 'og_calc_inputs_research_v2',
    INPUTS_MOON_BUILD: 'og_calc_inputs_moon_build_v2', TM: 'og_calc_tm_v2',
    BOXES: 'og_calc_boxes_v2', SHIP_QTY: 'og_calc_ship_qty_v2',
    ACTIVE_TAB: 'og_calc_active_tab_v2', LF_INPUTS_BUILD: 'og_calc_lf_inputs_build_v1',
    LF_INPUTS_RESEARCH: 'og_calc_lf_inputs_research_v1', LF_RACE: 'og_calc_lf_race_v1',
  ...
  const MOON_BUILDINGS_ICONS = ['lunar_base.png', 'sensor_phalanx.png', 'jump_gate.png'];
  ...
  function clearOldStorageData() {
    ...
    Object.keys(localStorage).forEach(key => {
      if (key.startsWith('og_calc_')) {
        try {
          const data = JSON.parse(localStorage.getItem(key));
          if (data && data.timestamp && now - data.timestamp > thirtyDays) localStorage.removeItem(key);
  ...
  const formatNumberWithDots = (n) => (n === null || n === undefined || isNaN(n)) ? '0' : formatWithDotsRaw(Math.round(Number(n) || 0));
    ...
    tbody.querySelectorAll('tr').forEach(tr => {
      ...
      if (!techId || !TECH_COSTS[techId]) {
        ['m', 'c', 'd'].forEach(cls => {
          ...
          cell.innerHTML = '';
          cell.appendChild(cls === 'm' ? formatSpanMetal(0) : cls === 'c' ? formatSpanCrystal(0) : formatSpanDeut(0));
        });
      ...
      let m = Math.round(resCost[0] * planets), c = Math.round(resCost[1] * planets), d = Math.round(resCost[2] * planets), p = Math.round(points * planets);
      tr.querySelector('td.m').innerHTML = ''; tr.querySelector('td.m').appendChild(formatSpanMetal(m));
      tr.querySelector('td.c').innerHTML = ''; tr.querySelector('td.c').appendChild(formatSpanCrystal(c));
      tr.querySelector('td.d').innerHTML = ''; tr.querySelector('td.d').appendChild(formatSpanDeut(d));
      tr.querySelector('td.p').textContent = formatNumberWithDots(p);
      tm += m; tc += c; td += d; tp += p;
    });
    document.getElementById('sumMetalLfB').innerHTML = ''; document.getElementById('sumMetalLfB').appendChild(formatSpanMetal(tm));
    document.getElementById('sumCrystalLfB').innerHTML = ''; document.getElementById('sumCrystalLfB').appendChild(formatSpanCrystal(tc));
    document.getElementById('sumDeutLfB').innerHTML = ''; document.getElementById('sumDeutLfB').appendChild(formatSpanDeut(td));
  ...
  function recalcAllLfResearch() {
    ...
    const rsrCostRdc = 0.0025 * levelValue;
    clearCalculationCache();
    tbody.querySelectorAll('tr').forEach(tr => {
      const techId = Number(tr.dataset.techId) || 0;
      if (!techId || !TECH_COSTS[techId]) {
      ...
      let m = Math.round(resCost[0] * planets), c = Math.round(resCost[1] * planets), d = Math.round(resCost[2] * planets);
      tr.querySelector('td.m').innerHTML = ''; tr.querySelector('td.m').appendChild(formatSpanMetal(m));
      tr.querySelector('td.c').innerHTML = ''; tr.querySelector('td.c').appendChild(formatSpanCrystal(c));
      tr.querySelector('td.d').innerHTML = ''; tr.querySelector('td.d').appendChild(formatSpanDeut(d));
      tr.querySelector('td.p').textContent = formatNumberWithDots(p);
    ...
    document.getElementById('sumMetalLfR').innerHTML = ''; document.getElementById('sumMetalLfR').appendChild(formatSpanMetal(tm));
    document.getElementById('sumCrystalLfR').innerHTML = ''; document.getElementById('sumCrystalLfR').appendChild(formatSpanCrystal(tc));
    document.getElementById('sumDeutLfR').innerHTML = ''; document.getElementById('sumDeutLfR').appendChild(formatSpanDeut(td));
  ...
  function buildRowsMoonBuildings() {
    ...
    MOON_BUILDINGS_DATA.forEach((building, i) => {
      const name = dict[`moon_${building.id}`] || building.name;
      const tr = document.createElement('tr');
      tr.dataset.index = i; tr.dataset.buildingId = building.id;
      const tdName = document.createElement('td'); tdName.className = 'name-cell';
      const img = document.createElement('img');
      img.src = `${IMAGES_BUILDINGS_PATH}${MOON_BUILDINGS_ICONS[i]}`; img.className = 'icon'; img.alt = name;
      img.addEventListener('error', () => {
        if (!img._fallback) {
          const fb = createImageFallbackEl(name); img.style.display = 'none';
          if (img.parentNode) img.parentNode.insertBefore(fb, img.nextSibling);
          img._fallback = true;
        }
      });
      tdName.appendChild(img); tdName.appendChild(document.createTextNode(name)); tr.appendChild(tdName);
      const tdFrom = document.createElement('td');
      const fromInput = document.createElement('input'); fromInput.type = 'text'; fromInput.className = 'lvl-input'; fromInput.dataset.type = 'from'; fromInput.dataset.index = i; fromInput.inputmode = 'numeric';
      tdFrom.appendChild(fromInput);
      const tdTo = document.createElement('td');
      const toInput = document.createElement('input'); toInput.type = 'text'; toInput.className = 'lvl-input'; toInput.dataset.type = 'to'; toInput.dataset.index = i; toInput.inputmode = 'numeric';
      tdTo.appendChild(toInput);
      const tdMoons = document.createElement('td');
      const moonInput = document.createElement('input'); moonInput.type = 'text'; moonInput.className = 'planet-input'; moonInput.dataset.type = 'moons'; moonInput.dataset.index = i; moonInput.inputmode = 'numeric'; moonInput.value = '1';
      tdMoons.appendChild(moonInput);
      ...
      tr.append(tdFrom, tdTo, tdMoons, tdM, tdC, tdD, tdP); frag.appendChild(tr);
    });
    tbody.appendChild(frag); attachLvlInputHandlers();
  }

  function recalcAllMoonBuildings() {
    const tbody = document.getElementById('tbodyMoonBuildings');
    if (!tbody) return;
    let tm = 0, tc = 0, td = 0, tp = 0;
    clearCalculationCache();
    tbody.querySelectorAll('tr').forEach(tr => {
      ...
      const data = MOON_BUILDINGS_DATA[idx];
      if (!data) return;
      ...
      to = Math.max(from, to);
      if (to - from > CONFIG.MAX_LEVEL_SPAN) to = from + CONFIG.MAX_LEVEL_SPAN;
      const moons = Math.max(1, parseNumberInput(tr.querySelector('input[data-type="moons"]')?.value) || 1);
      const sum = geomSum({ m: data.base.m, c: data.base.c, d: data.base.d }, data.factor, from, to);
      let m = Math.round(sum.m * moons), c = Math.round(sum.c * moons), d = Math.round(sum.d * moons), p = Math.round(sum.points * moons);
      tr.querySelector('td.m').innerHTML = ''; tr.querySelector('td.m').appendChild(formatSpanMetal(m));
      tr.querySelector('td.c').innerHTML = ''; tr.querySelector('td.c').appendChild(formatSpanCrystal(c));
      tr.querySelector('td.d').innerHTML = ''; tr.querySelector('td.d').appendChild(formatSpanDeut(d));
      tr.querySelector('td.p').textContent = formatNumberWithDots(p);
    ...
    document.getElementById('sumMetalMoon').innerHTML = ''; document.getElementById('sumMetalMoon').appendChild(formatSpanMetal(tm));
    document.getElementById('sumCrystalMoon').innerHTML = ''; document.getElementById('sumCrystalMoon').appendChild(formatSpanCrystal(tc));
    document.getElementById('sumDeutMoon').innerHTML = ''; document.getElementById('sumDeutMoon').appendChild(formatSpanDeut(td));
    document.getElementById('sumPointsMoon').textContent = formatNumberWithDots(tp);
    document.getElementById('sumTotalMetalMoon').textContent = formatNumberWithDots(Math.round(convertToMetal(tm, tc, td)));
    updateBoxesNeeded();
  }
  ...
  function recalcAllBuildings() {
    ...
    tbodyB.querySelectorAll('tr').forEach(tr => {
      ...
      const planets = Math.max(1, parseNumberInput(tr.querySelector('input[data-type="planets"]')?.value) || 1);
      const sum = geomSum({ m: data.base.m, c: data.base.c, d: data.base.d }, data.factor, from, to);
      let m = Math.round(sum.m * planets), c = Math.round(sum.c * planets), d = Math.round(sum.d * planets), p = Math.round(sum.points * planets);
      if (currentLifeformRace === 'rocktal') {
        let totalDiscount = 0;
        ...
        if (totalDiscount > 0) {
          m = Math.ceil(m * (1 - totalDiscount)); c = Math.ceil(c * (1 - totalDiscount)); d = Math.ceil(d * (1 - totalDiscount));
    ...
    document.getElementById('sumMetalB').innerHTML = ''; document.getElementById('sumMetalB').appendChild(formatSpanMetal(tm));
    document.getElementById('sumCrystalB').innerHTML = ''; document.getElementById('sumCrystalB').appendChild(formatSpanCrystal(tc));
    document.getElementById('sumDeutB').innerHTML = ''; document.getElementById('sumDeutB').appendChild(formatSpanDeut(td));
  ...
  function recalcAllResearch() {
    ...
    document.getElementById('sumMetalR').innerHTML = ''; document.getElementById('sumMetalR').appendChild(formatSpanMetal(sm));
    document.getElementById('sumCrystalR').innerHTML = ''; document.getElementById('sumCrystalR').appendChild(formatSpanCrystal(sc));
    document.getElementById('sumDeutR').innerHTML = ''; document.getElementById('sumDeutR').appendChild(formatSpanDeut(sd));
    ...
    try {
      ...
      if (tbody) {
        ...
        if (rows.length >= 2) {
          ...
          sumRow.querySelector('td.m').innerHTML = ''; sumRow.querySelector('td.m').appendChild(formatSpanMetal(fleetM));
          sumRow.querySelector('td.c').innerHTML = ''; sumRow.querySelector('td.c').appendChild(formatSpanCrystal(fleetC));
          sumRow.querySelector('td.d').innerHTML = ''; sumRow.querySelector('td.d').appendChild(formatSpanDeut(fleetD));
          ...
          totalRow.querySelector('#sumTotalMetalF').textContent = formatNumberWithDots(totalMetal);
        }
      }
      updateBoxesNeeded();
  ...
  function renderTable() {
    try {
      ...
      shipList.forEach(ship => {
        const v = qtyMap[ship.id.trim()] || '';
        const row = document.createElement('tr');
        row.setAttribute('data-row-id', ship.id.trim());
        const lang = safeLocalStorageGet(KEYS.LANG, 'ru');
        const dict = window.getLangDict ? window.getLangDict(lang) : {};
        const shipName = dict[`ship_${ship.id.trim()}`] || ship.id.trim();
        const tdName = document.createElement('td'); tdName.style.textAlign = 'left';
        const img = document.createElement('img');
        img.src = `${IMAGES_SHIPS_PATH}${ship.img.trim()}`; img.alt = shipName; img.width = 28; img.height = 28; img.loading = 'lazy';
        img.style.cssText = 'width:28px;height:28px;vertical-align:middle;margin-right:8px;border-radius:4px;';
        img.addEventListener('error', () => {
          if (!img._fallback) {
            const fb = createImageFallbackEl(shipName); img.style.display = 'none';
            if (img.parentNode) img.parentNode.insertBefore(fb, img.nextSibling);
            img._fallback = true;
          }
        });
        tdName.appendChild(img);
        const span = document.createElement('span'); span.className = 'ship-name'; span.textContent = shipName;
        tdName.appendChild(span);
        const tdQty = document.createElement('td');
        const qtyInput = document.createElement('input'); qtyInput.type = 'text'; qtyInput.inputmode = 'numeric'; qtyInput.pattern = '[0-9.]*'; qtyInput.value = v ? formatWithDotsRaw(v) : ''; qtyInput.dataset.id = ship.id.trim();
      ...
      const lang = safeLocalStorageGet(KEYS.LANG, 'ru');
      const dict = window.getLangDict ? window.getLangDict(lang) : {};
      tdNameSum.textContent = dict.total || 'Итого';
      ...
      tdNameTotal.textContent = dict.totalInMetal || 'Всего в металле';
      const tdQtyTotal = document.createElement('td');
      const tdMTotal = document.createElement('td'); tdMTotal.className = 'm'; tdMTotal.colSpan = 3;
      const spanTotalMetal = document.createElement('span'); spanTotalMetal.id = 'sumTotalMetalF'; spanTotalMetal.textContent = '0';
  ...
  function buildRowsBuildings() {
    ...
    customOrder.forEach((i) => {
      ...
      const tdName = document.createElement('td'); tdName.className = 'name-cell';
      const iconFileName = ICONS_BUILDINGS[i].trim();
      if (iconFileName) {
        const img = document.createElement('img'); img.src = `${IMAGES_BUILDINGS_PATH}${iconFileName}`; img.className = 'icon'; img.alt = '';
        img.addEventListener('error', () => {
          if (!img._fallback) { const fb = createImageFallbackEl(name); img.style.display = 'none'; if (img.parentNode) img.parentNode.insertBefore(fb, img.nextSibling); img._fallback = true; }
        });
        tdName.appendChild(img);
      } else { tdName.appendChild(createImageFallbackEl(name)); }
      tdName.appendChild(document.createTextNode(name)); tr.appendChild(tdName);
      const tdFrom = document.createElement('td');
      const fromInput = document.createElement('input'); fromInput.type = 'text'; fromInput.className = 'lvl-input'; fromInput.dataset.type = 'from'; fromInput.dataset.index = i; fromInput.inputmode = 'numeric';
      tdFrom.appendChild(fromInput);
      const tdTo = document.createElement('td');
      ...
      const planetImg = document.createElement('img'); planetImg.src = `${IMAGES_ROOT_PATH}planet.png`; planetImg.className = 'icon'; planetImg.alt = '';
      const planetInput = document.createElement('input'); planetInput.type = 'text'; planetInput.className = 'planet-input'; planetInput.dataset.type = 'planets'; planetInput.dataset.index = i; planetInput.inputmode = 'numeric'; planetInput.value = '1';
      tdPlanets.append(planetImg, planetInput);
      const tdM = document.createElement('td'); tdM.className = 'm'; tdM.appendChild(formatSpanMetal(0));
      const tdC = document.createElement('td'); tdC.className = 'c'; tdC.appendChild(formatSpanCrystal(0));
      const tdD = document.createElement('td'); tdD.className = 'd'; tdD.appendChild(formatSpanDeut(0));
      const tdP = document.createElement('td'); tdP.className = 'p'; tdP.textContent = '0';
      tr.append(tdFrom, tdTo, tdPlanets, tdM, tdC, tdD, tdP); frag.appendChild(tr);
    });
    tbodyB.appendChild(frag); attachLvlInputHandlers();
  }
  ...
  function buildRowsResearch() {
    ...
    names.forEach((name, i) => {
      ...
      const tr = document.createElement('tr'); tr.dataset.index = i;
      const tdName = document.createElement('td'); tdName.className = 'name-cell';
      const iconFileName = ICONS_RESEARCH[i].trim();
      if (iconFileName) {
        const img = document.createElement('img'); img.src = `${IMAGES_RESEARCH_PATH}${iconFileName}`; img.className = 'icon'; img.alt = '';
        img.addEventListener('error', () => {
          if (!img._fallback) { const fb = createImageFallbackEl(cleanName); img.style.display = 'none'; if (img.parentNode) img.parentNode.insertBefore(fb, img.nextSibling); img._fallback = true; }
        });
        tdName.appendChild(img);
      } else { tdName.appendChild(createImageFallbackEl(cleanName)); }
      tdName.appendChild(document.createTextNode(cleanName)); tr.appendChild(tdName);
      const tdFrom = document.createElement('td');
      const fromInput = document.createElement('input'); fromInput.type = 'text'; fromInput.className = 'lvl-input'; fromInput.dataset.type = 'from'; fromInput.dataset.index = i; fromInput.inputmode = 'numeric';
      tdFrom.appendChild(fromInput);
      const tdTo = document.createElement('td');
      ...
      const tdD = document.createElement('td'); tdD.className = 'd'; tdD.appendChild(formatSpanDeut(0));
      const tdP = document.createElement('td'); tdP.className = 'p'; tdP.textContent = '0';
      tr.append(tdFrom, tdTo, tdM, tdC, tdD, tdP); frag.appendChild(tr);
    });
    tbodyR.appendChild(frag); attachLvlInputHandlers();
  }
  ...
  function buildRowsLfBuildings() {
    ...
    for (let i = 1; i <= 12; i++) {
      ...
      const tr = document.createElement('tr'); tr.dataset.index = i - 1; tr.dataset.techId = techId;
      const lang = safeLocalStorageGet(KEYS.LANG, 'ru');
      const dict = window.getLangDict ? window.getLangDict(lang) : {};
      ...
      const fileName = LF_BUILDING_FILENAMES[techId]?.trim() || `${techId}.png`;
      img.src = `${imagePath}${fileName}`; img.className = 'icon'; img.alt = name;
      img.addEventListener('error', () => {
        if (!img._fallback) { const fb = createImageFallbackEl(name); img.style.display = 'none'; if (img.parentNode) img.parentNode.insertBefore(fb, img.nextSibling); img._fallback = true; }
      });
      tdName.appendChild(img);
      const span = document.createElement('span'); span.textContent = name; tdName.appendChild(span); tr.appendChild(tdName);
      const tdFrom = document.createElement('td');
      const fromInput = document.createElement('input'); fromInput.type = 'text'; fromInput.className = 'lvl-input'; fromInput.dataset.type = 'from'; fromInput.dataset.index = i - 1;
      tdFrom.appendChild(fromInput);
      const tdTo = document.createElement('td');
      const toInput = document.createElement('input'); toInput.type = 'text'; toInput.className = 'lvl-input'; toInput.dataset.type = 'to'; toInput.dataset.index = i - 1;
      tdTo.appendChild(toInput);
      const tdPlanets = document.createElement('td');
      const planetImg = document.createElement('img'); planetImg.src = `${IMAGES_ROOT_PATH}planet.png`; planetImg.className = 'icon'; planetImg.alt = '';
      const planetInput = document.createElement('input'); planetInput.type = 'text'; planetInput.className = 'planet-input'; planetInput.dataset.type = 'planets'; planetInput.dataset.index = i - 1; planetInput.inputmode = 'numeric'; planetInput.value = '1';
      tdPlanets.append(planetImg, planetInput);
      const tdM = document.createElement('td'); tdM.className = 'm'; tdM.appendChild(formatSpanMetal(0));
      const tdC = document.createElement('td'); tdC.className = 'c'; tdC.appendChild(formatSpanCrystal(0));
      const tdD = document.createElement('td'); tdD.className = 'd'; tdD.appendChild(formatSpanDeut(0));
  ...
  function buildRowsLfResearch() {
    ...
    for (let i = 1; i <= 18; i++) {
      ...
      const fileName = LF_RESEARCH_FILENAMES[techId]?.trim() || `${techId}.png`;
      img.src = `${imagePath}${fileName}`; img.className = 'icon'; img.alt = name;
      img.addEventListener('error', () => {
        if (!img._fallback) { const fb = createImageFallbackEl(name); img.style.display = 'none'; if (img.parentNode) img.parentNode.insertBefore(fb, img.nextSibling); img._fallback = true; }
      });
  ...
  function saveShipQuantities() {
    try {
      const qtyMap = {};
      document.querySelectorAll('input[data-id]').forEach(inp => { qtyMap[inp.dataset.id] = parseNumberInput(sanitizeInput(inp.value)); });
      safeLocalStorageSet(KEYS.SHIP_QTY, JSON.stringify(qtyMap));
    } catch (e) { console.warn('Failed to save ship quantities:', e); }
  }
  ...
  class PanZoomController {
    constructor(wrapperId, handleId, storageKey) {
      ...
      this.minScale = 0.3; this.maxScale = 3.5; this.padding = 60;
      this.loadState(); this.applyTransform(); this._bindDrag();
    }
    loadState() {
      try {
        ...
        if (saved) {
          ...
          this.state = { x: Number(data.x) || 0, y: Number(data.y) || 0, scale: Math.max(this.minScale, Math.min(this.maxScale, Number(data.scale) || 1)) };
        }
      } catch (e) {}
    }
    saveState() { try { localStorage.setItem(this.storageKey, JSON.stringify(this.state)); } catch (e) {} }
    applyTransform() {
    ...
    zoom(factor, centerPoint = null) {
      ...
      this.clampPosition(); this.wrapper.style.willChange = 'transform'; this.applyTransform(); this.saveState();
      setTimeout(() => { if (!this.isDragging) this.wrapper.style.willChange = 'auto'; }, 150);
    }
    reset() { this.state = { x: 0, y: 0, scale: 1 }; this.clampPosition(); this.applyTransform(); this.saveState(); }
    center() {
      const rect = this.wrapper.getBoundingClientRect(), s = this.state.scale;
      const baseW = rect.width / s, baseH = rect.height / s;
      this.state.x = Math.round((window.innerWidth - baseW) / 2);
      this.state.y = Math.round((window.innerHeight - baseH) / 2);
      this.applyTransform(); this.saveState();
    }
    _bindDrag() {
      this.handle.style.touchAction = 'none'; this.handle.style.cursor = 'grab';
      this._onPointerDown = (e) => {
        this.isDragging = true; this.startPointer = { x: e.clientX, y: e.clientY }; this.startState = { ...this.state };
        this.handle.style.cursor = 'grabbing'; this.wrapper.style.zIndex = '2000'; this.wrapper.style.willChange = 'transform';
      ...
      this._onPointerMove = (e) => {
        ...
        this.state.x = this.startState.x + dx; this.state.y = this.startState.y + dy;
        this.applyTransform();
      };
      this._stopDrag = (e) => {
        if (!this.isDragging) return;
        this.isDragging = false; this.handle.style.cursor = 'grab'; this.wrapper.style.zIndex = '1000'; this.wrapper.style.willChange = 'auto';
        this.clampPosition(); this.applyTransform(); this.saveState();
    ...
    destroy() {
      ...
      if (this._stopDrag) { document.removeEventListener('pointerup', this._stopDrag); document.removeEventListener('pointercancel', this._stopDrag); }
  ...
  function attachInputsHandlers() {
    ...
    const handlers = {
      buildings: () => recalcAllBuildings(), research: () => recalcAllResearch(), fleet: () => computeFleet(),
      lfBuildings: () => recalcAllLfBuildings(), lfResearch: () => recalcAllLfResearch(), moonBuildings: () => recalcAllMoonBuildings()
    };
    const debounced = {};
    for (const k in handlers) debounced[k] = debounce(handlers[k], 150);
    const sumCheckbox = document.getElementById('sumAllTabsCheckbox');
    if (sumCheckbox) sumCheckbox.addEventListener('change', (e) => {
      isSumAllTabsMode = e.target.checked;
      safeLocalStorageSet(KEYS.SUM_ALL_TABS, String(isSumAllTabsMode));
      updateBoxesNeeded();
    });
    const persistTable = (tbodyId, key) => {
    ...
    const bindTable = (tbodyId, type, saveKey) => {
      const tbody = document.getElementById(tbodyId);
      if (!tbody) return;
      const getDynamicKey = () => {
        if (!saveKey) return null;
        if (saveKey.includes(KEYS.LF_INPUTS_BUILD)) return `${KEYS.LF_INPUTS_BUILD}_${currentLifeformRace}`;
        if (saveKey.includes(KEYS.LF_INPUTS_RESEARCH)) return `${KEYS.LF_INPUTS_RESEARCH}_${currentLifeformRace}`;
        return saveKey;
      };
      tbody.addEventListener('input', (e) => {
        if (!e.target.matches('.lvl-input, .planet-input')) return;
        debounced[type]();
        const key = getDynamicKey();
        if (key) persistTable(tbodyId, key);
      });
      tbody.addEventListener('change', () => { debounced[type](); const key = getDynamicKey(); if (key) persistTable(tbodyId, key); });
    };
    bindTable('tbodyBuildings', 'buildings', KEYS.INPUTS_BUILD);
    bindTable('tbodyResearch', 'research', KEYS.INPUTS_RESEARCH);
    bindTable('tbodyMoonBuildings', 'moonBuildings', KEYS.INPUTS_MOON_BUILD);
    bindTable('tbodyLfBuildings', 'lfBuildings', `${KEYS.LF_INPUTS_BUILD}_${currentLifeformRace}`);
    bindTable('tbodyLfResearch', 'lfResearch', `${KEYS.LF_INPUTS_RESEARCH}_${currentLifeformRace}`);
    const tmEl = document.getElementById('tmInput');
    if (tmEl) {
      tmEl.addEventListener('input', () => debounced.research());
      tmEl.addEventListener('blur', () => debounced.research());
    ...
    if (sel) {
      sel.addEventListener('change', (e) => {
        ...
        buildRowsLfBuildings(); buildRowsLfResearch();
        attachLiveThousandsFormatting('#tbodyLfBuildings input, #tbodyLfResearch input');
        restoreAllInputsAfterSwitch();
        recalcAllLfBuildings(); recalcAllLfResearch(); updateBoxesNeeded();
      });
    }
    document.querySelectorAll('#tabsLeft .tab-btn').forEach(btn => {
      btn.addEventListener('click', (ev) => {
        ev.stopPropagation();
        document.querySelectorAll('#tabsLeft .tab-btn').forEach(b => { b.classList.remove('active'); b.setAttribute('aria-selected', 'false'); b.setAttribute('tabindex', '-1'); });
        btn.classList.add('active'); btn.setAttribute('aria-selected', 'true'); btn.setAttribute('tabindex', '0');
        document.querySelectorAll('.tab-content').forEach(p => p.classList.remove('active'));
        setActiveTab(btn.dataset.tab); ensureProperPositioning();
      });
    });
    document.querySelectorAll('.lf-subtab-btn').forEach(btn => {
      btn.addEventListener('click', (ev) => {
        ev.stopPropagation(); persistLfInputs();
        ...
        if (subtab === 'lf-buildings') recalcAllLfBuildings(); else recalcAllLfResearch();
        updateBoxesNeeded(); safeLocalStorageSet('og_calc_active_lf_subtab_v1', subtab);
      });
    });
    document.querySelectorAll('.building-subtab-btn').forEach(btn => {
      btn.addEventListener('click', (ev) => {
        ...
        try { if (tab === 'moon') recalcAllMoonBuildings(); else recalcAllBuildings(); } catch (e) { console.error('Tab switch recalc error:', e); }
        updateBoxesNeeded(); safeLocalStorageSet('og_calc_active_building_tab', tab);
      });
    });
    ...
    if (langToggle && langDropdownMenu) {
      ...
      availableLangs.forEach(langCode => {
        const option = document.createElement('div'); option.className = 'lang-option'; option.dataset.lang = langCode;
        const iconImg = document.createElement('img'); iconImg.src = LANGUAGE_ICONS[langCode] || 'images/languages/unknown.png'; iconImg.alt = LANGUAGE_NAMES[langCode] || langCode.toUpperCase(); iconImg.className = 'lang-icon-img'; iconImg.style.cssText = 'width:18px;height:18px;vertical-align:middle;margin-right:6px;';
        const textSpan = document.createElement('span'); textSpan.className = 'lang-text'; textSpan.textContent = LANGUAGE_NAMES[langCode] || langCode.toUpperCase();
        option.append(iconImg, textSpan); langDropdownMenu.appendChild(option);
        option.addEventListener('click', (e) => {
          e.stopPropagation(); e.preventDefault(); applyLang(langCode);
          setTimeout(() => { langDropdownMenu.style.display = 'none'; }, 50);
          currentLangEl.innerHTML = '';
          const newIconImg = document.createElement('img'); newIconImg.src = LANGUAGE_ICONS[langCode] || 'images/languages/unknown.png'; newIconImg.alt = LANGUAGE_NAMES[langCode] || langCode.toUpperCase(); newIconImg.className = 'lang-icon-img'; newIconImg.style.cssText = 'width:18px;height:18px;vertical-align:middle;margin-right:4px;';
          const newTextSpan = document.createElement('span'); newTextSpan.className = 'lang-text'; newTextSpan.textContent = LANGUAGE_NAMES[langCode] || langCode.toUpperCase();
      ...
      if (currentLangEl) {
        currentLangEl.innerHTML = '';
        const iconImg = document.createElement('img'); iconImg.src = LANGUAGE_ICONS[currentLang] || 'images/languages/unknown.png'; iconImg.alt = LANGUAGE_NAMES[currentLang] || currentLang.toUpperCase(); iconImg.className = 'lang-icon-img'; iconImg.style.cssText = 'width:18px;height:18px;vertical-align:middle;margin-right:4px;';
        const textSpan = document.createElement('span'); textSpan.className = 'lang-text'; textSpan.textContent = LANGUAGE_NAMES[currentLang] || currentLang.toUpperCase();
        currentLangEl.append(iconImg, textSpan);
      }
      langToggle.addEventListener('click', (e) => { e.stopPropagation(); e.preventDefault(); langDropdownMenu.style.display = langDropdownMenu.style.display === 'flex' ? 'none' : 'flex'; });
      document.addEventListener('click', (e) => { if (!langDropdownMenu.contains(e.target) && e.target !== langToggle) langDropdownMenu.style.display = 'none'; });
    }
    ...
    BONUS_INPUT_IDS.forEach(id => {
      ...
      if (el && !el._bonusBound) {
        const handler = () => {
          ...
          else if (id === 'megalithLevel' || id === 'mrcLevel') { recalcAllLfBuildings(); recalcAllBuildings(); }
          else if (id === 'runoLevel') recalcAllLfResearch();
          updateBoxesNeeded();
          safeLocalStorageSet(`og_calc_${id}`, String(parseNumberInput(el.value)));
        };
        el.addEventListener('input', handler); el.addEventListener('change', handler); el.addEventListener('blur', handler);
        el._bonusBound = true;
      }
    });
  }
  ...
  function persistLfInputs() {
    ...
    } catch (e) { console.warn('Persist LF inputs:', e); }
  }
  ...
  function applyLang(lang, skipRebuild = false) {
    ...
    document.querySelectorAll("[data-i18n-ph]").forEach(el => { const key = el.getAttribute("data-i18n-ph"); if (dict[key]) el.setAttribute("placeholder", dict[key]); });
    ...
    if (lfSelect) {
      ...
      options.forEach(opt => { if (dict[opt.value]) opt.textContent = dict[opt.value]; });
    ...
    if (!skipRebuild) {
      ...
      buildRowsBuildings(); buildRowsResearch(); buildRowsMoonBuildings(); renderTable(); buildRowsLfBuildings(); buildRowsLfResearch();
      restoreAllInputsAfterSwitch(); setActiveTab(activeTab);
      ...
      recalcAllBuildings(); recalcAllResearch(); recalcAllMoonBuildings(); recalcAllLfBuildings(); recalcAllLfResearch(); computeFleet(); updateBoxesNeeded();
      updateLfBonusesVisibility(currentLifeformRace);
  ...
  function saveAllInputsBeforeSwitch() {
    try {
      ...
      saveInputRows('#tbodyMoonBuildings tr', KEYS.INPUTS_MOON_BUILD);
      saveInputRows('#tbodyLfBuildings tr', `${KEYS.LF_INPUTS_BUILD}_${currentLifeformRace}`);
      saveInputRows('#tbodyLfResearch tr', `${KEYS.LF_INPUTS_RESEARCH}_${currentLifeformRace}`);
      saveShipQuantities();
      ...
      BONUS_INPUT_IDS.forEach(id => { const el = document.getElementById(id); if (el) safeLocalStorageSet(`og_calc_${id}`, el.value); });
    } catch (e) { console.warn('Save all inputs:', e); }
  }
  ...
  function restoreAllInputsAfterSwitch() {
    try {
      const restoreRows = (selector, key) => {
        ...
        document.querySelectorAll(selector).forEach((tr, i) => {
          if (data[i]) {
            ...
            const planetInp = tr.querySelector('input[data-type="planets"]');
      ...
      restoreRows('#tbodyMoonBuildings tr', KEYS.INPUTS_MOON_BUILD);
      restoreRows('#tbodyLfBuildings tr', `${KEYS.LF_INPUTS_BUILD}_${currentLifeformRace}`);
      restoreRows('#tbodyLfResearch tr', `${KEYS.LF_INPUTS_RESEARCH}_${currentLifeformRace}`);
      const shipQty = JSON.parse(safeLocalStorageGet(KEYS.SHIP_QTY, '{}'));
      if (shipQty) document.querySelectorAll('input[data-id]').forEach(inp => { const v = shipQty[inp.dataset.id]; if (v) inp.value = formatWithDotsRaw(v); });
      ...
      BONUS_INPUT_IDS.forEach(id => { const saved = safeLocalStorageGet(`og_calc_${id}`, null); if (saved !== null && document.getElementById(id)) document.getElementById(id).value = String(parseNumberInput(saved)); });
    } catch (e) { console.warn('Restore inputs:', e); }
  }
    ...
    window.requestAnimationFrame(() => {
      window.requestAnimationFrame(() => {
        ...
        setTimeout(() => {
          ...
          if (wrapper && (wrapper.getBoundingClientRect().width === 0 || wrapper.getBoundingClientRect().height === 0)) positionTabs();
        }, 100);
      });
    });
  }
  ...
  function restoreFromStorage() {
    try {
      ...
      if (inputsBuild) {
        ...
        trs.forEach((tr, i) => {
          if (inputsBuild[i]) {
            const fromInp = tr.querySelector('input[data-type="from"]'), toInp = tr.querySelector('input[data-type="to"]'), planetInp = tr.querySelector('input[data-type="planets"]');
            if (fromInp) fromInp.value = inputsBuild[i].from ? String(Math.min(99, inputsBuild[i].from)) : '';
            if (toInp) toInp.value = inputsBuild[i].to ? String(Math.min(99, inputsBuild[i].to)) : '';
            if (planetInp) planetInp.value = inputsBuild[i].planets ? formatWithDotsRaw(inputsBuild[i].planets) : '1';
          }
        });
      }
      const inputsResearch = JSON.parse(safeLocalStorageGet(KEYS.INPUTS_RESEARCH, 'null'));
      if (inputsResearch) {
        const trs = document.getElementById('tbodyResearch')?.querySelectorAll('tr') || [];
        trs.forEach((tr, i) => {
          if (inputsResearch[i]) {
            const fromInp = tr.querySelector('input[data-type="from"]'), toInp = tr.querySelector('input[data-type="to"]');
            if (fromInp) fromInp.value = inputsResearch[i].from ? String(Math.min(99, inputsResearch[i].from)) : '';
            if (toInp) toInp.value = inputsResearch[i].to ? String(Math.min(99, inputsResearch[i].to)) : '';
          }
        });
      }
      const inputsMoonBuild = JSON.parse(safeLocalStorageGet(KEYS.INPUTS_MOON_BUILD, 'null'));
      if (inputsMoonBuild) {
        const trs = document.getElementById('tbodyMoonBuildings')?.querySelectorAll('tr') || [];
        trs.forEach((tr, i) => {
          if (inputsMoonBuild[i]) {
            const fromInp = tr.querySelector('input[data-type="from"]'), toInp = tr.querySelector('input[data-type="to"]'), moonInp = tr.querySelector('input[data-type="moons"]');
            if (fromInp) fromInp.value = inputsMoonBuild[i].from ? String(Math.min(99, inputsMoonBuild[i].from)) : '';
            if (toInp) toInp.value = inputsMoonBuild[i].to ? String(Math.min(99, inputsMoonBuild[i].to)) : '';
            if (moonInp) moonInp.value = inputsMoonBuild[i].planets ? formatWithDotsRaw(inputsMoonBuild[i].planets) : '1';
          }
        });
      }
      ...
      if (bonusesEl) { bonusesEl.innerHTML = ''; updateLfBonusesVisibility(savedRace); }
      const lfInputsBuild = JSON.parse(safeLocalStorageGet(`${KEYS.LF_INPUTS_BUILD}_${savedRace}`, 'null'));
      if (lfInputsBuild) {
        document.querySelectorAll('#tbodyLfBuildings tr').forEach((tr, i) => {
          if (lfInputsBuild[i]) {
            const fromInp = tr.querySelector('input[data-type="from"]'), toInp = tr.querySelector('input[data-type="to"]'), planetInp = tr.querySelector('input[data-type="planets"]');
            if (fromInp) fromInp.value = lfInputsBuild[i].from ? String(Math.min(99, lfInputsBuild[i].from)) : '';
            if (toInp) toInp.value = lfInputsBuild[i].to ? String(Math.min(99, lfInputsBuild[i].to)) : '';
            if (planetInp) planetInp.value = lfInputsBuild[i].planets ? formatWithDotsRaw(lfInputsBuild[i].planets) : '1';
          }
        });
      }
      const lfInputsResearch = JSON.parse(safeLocalStorageGet(`${KEYS.LF_INPUTS_RESEARCH}_${savedRace}`, 'null'));
      if (lfInputsResearch) {
        document.querySelectorAll('#tbodyLfResearch tr').forEach((tr, i) => {
          if (lfInputsResearch[i]) {
            const fromInp = tr.querySelector('input[data-type="from"]'), toInp = tr.querySelector('input[data-type="to"]'), planetInp = tr.querySelector('input[data-type="planets"]');
            if (fromInp) fromInp.value = lfInputsResearch[i].from ? String(Math.min(99, lfInputsResearch[i].from)) : '';
            if (toInp) toInp.value = lfInputsResearch[i].to ? String(Math.min(99, lfInputsResearch[i].to)) : '';
            if (planetInp) planetInp.value = lfInputsResearch[i].planets ? formatWithDotsRaw(lfInputsResearch[i].planets) : '1';
          }
        });
      }
      const boxes = JSON.parse(safeLocalStorageGet(KEYS.BOXES, '{}'));
    ...
    } catch (e) { console.error('Restore from storage:', e); }
  }

  function resetAndCenter() {
    fullResetToZero();
    setTimeout(() => {
      const isExpeditions = document.getElementById('expeditionsWrapper')?.style.display !== 'none';
      if (isExpeditions) { if (window.panZoomExpeditions) window.panZoomExpeditions.center(); }
      else { if (window.panZoomMain) window.panZoomMain.center(); }
    }, 100);
  }
  ...
  function positionTabs() {
    try {
      const tabsLeftEl = document.getElementById('tabsLeft'), headerBarEl = document.getElementById('headerBar'), wrapperEl = document.getElementById('tableWrapper');
      if (!tabsLeftEl || !headerBarEl || !wrapperEl) return;
      const boxRow = document.getElementById('globalBoxRow') || headerBarEl;
      const wrapperRect = wrapperEl.getBoundingClientRect(), targetRect = boxRow.getBoundingClientRect();
    ...
    } catch (e) { console.warn('Position tabs:', e); }
  }
  ...
  function updateBoxesCostTL(targetMetal = null) {
    try {
      ...
      const bynValue = formatNumberWithDots(Math.round(totalTRY / 16.01));
  ...
  function setActiveTab(tab) {
    ...
    const tabB = document.getElementById('tabBuildings'), tabR = document.getElementById('tabResearch'), tabF = document.getElementById('tabFleet'), tabL = document.getElementById('tabLifeforms');
    ...
    if (tab === 'fleet') { renderTable(); computeFleet(); }
    else {
      if (tab === 'buildings') recalcAllBuildings();
        ...
        buildRowsLfBuildings(); buildRowsLfResearch(); restoreAllInputsAfterSwitch();
        attachLiveThousandsFormatting('#tbodyLfBuildings input, #tbodyLfResearch input');
        recalcAllLfBuildings(); recalcAllLfResearch();
      }
    }
    updateBoxesNeeded(); safeLocalStorageSet(KEYS.ACTIVE_TAB, tab);
  }
  ...
  function initZoomControls() {
    const zoomIn = document.getElementById('globalZoomIn'), zoomOut = document.getElementById('globalZoomOut'), zoomReset = document.getElementById('globalZoomReset');
    ...
    if (zoomReset) zoomReset.addEventListener('click', () => {
      ...
      const isExp = document.getElementById('expeditionsWrapper')?.style.display !== 'none';
      if (isExp) {
        ['tech_hyper-level', 'percent-resources', 'percent-ships', 'class-bonus-collector', 'class-bonus-discoverer', 'dark-matter-discovery-bonus'].forEach(id => { const el = document.getElementById(id); if (el) el.value = '0'; });
  ...
  function updateBackgroundVideo(view) {
    ...
    bgVideo.pause(); bgVideo.src = targetSrc; bgVideo.load(); bgVideo.play().catch(() => {});
  }
  ...
  function updateLfBonusesVisibility(race) {
    ...
    } else if (race === 'humans') { fieldsToCreate = [{ labelKey: 'lf_b_1003', inputId: 'humansLevel', placeholder: '0' }]; }
    else if (race === 'mechas') { fieldsToCreate = [{ labelKey: 'lf_b_3003', inputId: 'mechasLevel', placeholder: '0' }]; }
    else if (race === 'kaelesh') { fieldsToCreate = [{ labelKey: 'lf_b_4003', inputId: 'kaeleshLevel', placeholder: '0' }]; }
    const frag = document.createDocumentFragment();
    fieldsToCreate.forEach(field => {
      const fieldDiv = document.createElement('div'); fieldDiv.className = 'field';
      const label = document.createElement('label'); label.setAttribute('for', field.inputId); label.textContent = dict[field.labelKey] || field.labelKey;
      const input = document.createElement('input'); input.id = field.inputId; input.type = 'text'; input.inputmode = 'numeric'; input.min = '0'; input.max = '100'; input.placeholder = field.placeholder;
      ...
      else { const savedValue = safeLocalStorageGet(`og_calc_${field.inputId}`, null); if (savedValue !== null) input.value = formatWithDotsRaw(parseNumberInput(savedValue)); }
      fieldDiv.append(label, input); frag.appendChild(fieldDiv);
    });
    ...
    BONUS_INPUT_IDS.forEach(id => {
      ...
      if (savedValue !== null && input && !input.value) input.value = formatWithDotsRaw(parseNumberInput(savedValue));
    });
  }
  ...
  function initApp() {
    try {
      ...
      document.querySelectorAll('.nav-btn').forEach(btn => { btn.classList.toggle('active', btn.dataset.view === savedView); });
      const tableWrapper = document.getElementById('tableWrapper'), expWrapper = document.getElementById('expeditionsWrapper');
