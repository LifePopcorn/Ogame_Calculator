...
const lang = safeLocalStorageGet(KEYS.LANG, 'ru');
const dict = window.getLangDict ? window.getLangDict(lang) : {};
const names = (typeof LANG_BUILDINGS !== 'undefined' && LANG_BUILDINGS[lang]) ? LANG_BUILDINGS[lang] : (typeof LANG_BUILDINGS !== 'undefined' ? LANG_BUILDINGS.ru : []);
...
if (iconFileName) {
const img = document.createElement('img'); img.src = `${IMAGES_BUILDINGS_PATH}${iconFileName}`; img.className = 'icon'; img.alt = name;
...
const lang = safeLocalStorageGet(KEYS.LANG, 'ru');
const dict = window.getLangDict ? window.getLangDict(lang) : {};
const names = (typeof LANG_RESEARCH !== 'undefined' && LANG_RESEARCH[lang]) ? LANG_RESEARCH[lang] : (typeof LANG_RESEARCH !== 'undefined' ? LANG_RESEARCH.ru : []);
const frag = document.createDocumentFragment();
...
if (iconFileName) {
const img = document.createElement('img'); img.src = `${IMAGES_RESEARCH_PATH}${iconFileName}`; img.className = 'icon'; img.alt = cleanName;
img.addEventListener('error', () => {
...
const tr = document.createElement('tr'); tr.dataset.index = i; tr.dataset.buildingId = building.id;
const tdName = document.createElement('td'); tdName.className = 'name-cell';
const img = document.createElement('img'); img.src = `${IMAGES_BUILDINGS_PATH}${MOON_BUILDINGS_ICONS[i]}`; img.className = 'icon'; img.alt = name;
img.addEventListener('error', () => {
if (!img._fallback) { const fb = createImageFallbackEl(name); img.style.display = 'none'; if (img.parentNode) img.parentNode.insertBefore(fb, img.nextSibling); img._fallback = true; }
});
...
const lang = safeLocalStorageGet(KEYS.LANG, 'ru');
const dict = window.getLangDict ? window.getLangDict(lang) : {};
const shipName = (dict && dict[`ship_${ship.id.trim()}`]) ? dict[`ship_${ship.id.trim()}`] : ship.id.trim();
...
saveShipQuantities(); computeFleet();
}
}, 150));
...
from: parseNumberInput(sanitizeInput(tr.querySelector('input[data-type="from"]')?.value)),
to: parseNumberInput(sanitizeInput(tr.querySelector('input[data-type="to"]')?.value)),
planets: parseNumberInput(sanitizeInput(tr.querySelector(`input[data-type="${selector.includes('Moon') ? 'moons' : 'planets'}"]`)?.value)) || 1
});
});
...
const dict = window.getLangDict ? window.getLangDict(lang) : {};
document.querySelectorAll('[data-i18n]').forEach(el => { const key = el.getAttribute('data-i18n'); if (dict && dict[key]) el.textContent = dict[key]; });
document.querySelectorAll('[data-i18n-ph]').forEach(el => { const key = el.getAttribute('data-i18n-ph'); if (dict && dict[key]) el.setAttribute('placeholder', dict[key]); });
...
const label = document.createElement('label'); label.setAttribute('for', field.inputId); label.textContent = (dict && dict[field.labelKey]) ? dict[field.labelKey] : field.labelKey;
